"""
Run War of Attrition auction simulation with learning agents
"""
import numpy as np
from matplotlib import pyplot as plt
from auctions import WarOfAttritionAuction, plot_results
from ppo_agent import PPOAgent

def run_war_of_attrition_simulation(n_agents: int, n_rounds: int):
    """
    Run War of Attrition auction simulation with PPO agents.

    Args:
        n_agents: Number of bidding agents
        n_rounds: Number of episodes (each with multiple auctions)
    """
    auction = WarOfAttritionAuction(n_agents)

    # Use PPO agents (same as in auctions.py)
    ...
    print(f"Agents: {n_agents}, Episodes: {n_rounds}, Auctions per episode: {auctions_per_episode}")
    print("=" * 60)
    ...
    for episode in range(n_rounds):
        ...
        for round_idx in range(auctions_per_episode):
            ...
            # Revenue: total payments (all agents pay second-highest bid)
            revenue_hist.append(outcome.winning_bid)  # This is total revenue / n_agents
...
def plot_results_war_of_attrition(n_agents, theta_hist, avg_theta_hist, efficiency_hist, revenue_hist):
    ...
    ax.set_xlabel('round')
    ax.set_ylabel('avg revenue per auction')
    ax.set_title('Auctioneer Revenue (Second-Highest Bid × N agents)')
    ax.grid(alpha=0.3)
