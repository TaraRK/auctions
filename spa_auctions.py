...
def run_simulation(n_agents: int, n_rounds: int, gamma: float):
    ...
    auction = SecondPriceAuction(n_agents)
...
def run_gamma_sweep(gamma_values, n_agents=10, n_rounds=1000, n_trials=3):
    ...
    # Write CSV header
    csv_filename = 'spa_07_gamma_sweep_results.csv'
...
def plot_gamma_vs_theta(results):
    ...
    plt.savefig('spa_gamma_vs_theta.png', dpi=150, bbox_inches='tight')
...
def new_plot_code(filename):
    ...
    ax.errorbar(df['gamma'], df['theta_mean'], yerr=df['theta_std'], 
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig('spa_gamma_vs_theta_new.png', dpi=150, bbox_inches='tight')

# new_plot_code('spa_gamma_sweep_results.csv')
