...
def run_simulation(n_agents: int, n_rounds: int, gamma: float):
    ...
    auctions_per_episode = 150 # for PPO agents
    agents = [PPOAgent(
        i, 
        initial_budget=3000.0,  # budget for one episode
...
def new_plot_code(filename):
    ...
    # Filter to only summary rows (where trial == 'summary')
    summary_df = df[df['trial'] == 'summary'].copy()
    
    # Convert to numeric (they're strings from CSV)
    summary_df['gamma'] = pd.to_numeric(summary_df['gamma'])
    summary_df['theta_mean'] = pd.to_numeric(summary_df['theta_mean'])
    summary_df['theta_std'] = pd.to_numeric(summary_df['theta_std'])
    
    # Sort by gamma for proper line plotting
    summary_df = summary_df.sort_values('gamma')

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.errorbar(summary_df['gamma'], summary_df['theta_mean'], yerr=summary_df['theta_std'], 
                marker='o', capsize=5, capthick=2, linewidth=2, markersize=8)
    ax.set_xlabel('gamma', fontsize=12)
    ax.set_ylabel('mean theta (bid/value)', fontsize=12)
    ax.set_title('Effect of Discount Factor on Bid Shading')
    ax.grid(alpha=0.3)
    ax.set_xscale('log')
    plt.tight_layout()
